...
class XMLIterator(TableIterator):
    ...
    def __init__(self, fd, model):
        ...
        self.related_fields = dict({
            #(f.name, f.rel.to) for f in self.model._meta.get_fields()              # gnedoy Django 2.0
            (f.name, f.remote_field.model) for f in self.model._meta.get_fields()   # gnedoy Django 2.0
